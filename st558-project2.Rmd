...
This first multiple regression (linear regression model extended to include more explanatory variables and/or higher order terms) will try to predict the number of shares based on all the available data in our training dataset.  

```{r multiple regression full fit, warning=FALSE}
# building the model
fullFit <- train(shares ~ ., 
              data = training, 
...
The next multiple regresson model includes a subset of variables that we think would be important. 

```{r multiple regression w/selected variables, warning=FALSE}
selectFit <- train(shares ~ n_tokens_content + num_hrefs + num_self_hrefs +
...
# Get rid of NA/missing values so train() will work
#trainRF <- na.omit(training)
#train model
rfFit <- train(x = select(training, -url, -shares), 
                y = training$shares,
                method = "rf",
                #na.action = na.omit, # not sure this is acceptable 
                tuneGrid = tunegrid,
                preProcess = c("center", "scale", "nzv"),
...
```{r boosted tree model, message=FALSE}
...
# Plot the parameters and accuracy levels
trellis.par.set(caretTheme())
plot(btFit) 

# use the tree to make predictions based on testing data
pred <- predict(btFit, newdata = testing)
```
...
selPred <- predict(selectFit, newdata = testing)
# random forest
...
data.frame(t(fullFit$results), 
           t(selectFit$results),
           t(rfFit$results),
           t(btFit$results))

# Select a model based on the lowest RMSE or highest R-squared value & display
